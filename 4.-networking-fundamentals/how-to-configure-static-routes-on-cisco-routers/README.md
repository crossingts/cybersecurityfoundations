...
To keep things simple in this here, we will assume that each device already knows its neighbor’s MAC address.
...
In this illustration we will use path 1, the path **via R3**. 

Using the techniques of load balancing and floating static routes, it is possible to configure the routers to use both paths. Instead of choosing one path, they can load-balance between path 1 and 2. That means some packets will take path 1 and some will take path 2. Or they could, for example, use path 1 as the main path and path 2 as a backup path.
...
* A static route is configured from global configuration mode using the `ip route` command. An IP address is configured on a router's interface from interface configuration mode using the `ip address` command.
