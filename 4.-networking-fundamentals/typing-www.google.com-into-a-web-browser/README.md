...
Before any data can be placed into an IP packet, it must first be encapsulated inside a Transport layer segment. The Transport layer (Layer 4) provides two essential services for process to process communication (service to service delivery): it uses port numbers to identify the specific source and destination applications, and it can offer reliability (through TCP) or a lightweight, connectionless service (through UDP). In our example, the DNS query from host A to host B is carried inside a UDP datagram destined to port 53, the standard DNS port. Later, after learning the web server’s IP address, host A will establish a TCP connection to host C’s port 80 (HTTP) or 443 (HTTPS) using a three‑way handshake (SYN, SYN‑ACK, ACK) before sending the HTTP GET request inside a TCP segment. These Transport layer headers – containing source and destination ports, sequence and acknowledgment numbers – are carried as the payload of the IP packets we trace in the following steps. While this walkthrough concentrates on how the Network and Data Link layers move those IP packets across routers and switches, remember that every IP packet contains a Transport layer segment that the endpoints use to deliver the data to the correct application and, in the case of TCP, to ensure it arrives reliably and in order.

Now we are ready for host A to send some data to host B.

Host A will start by adding an L3 header to its data to facilitate the end to end delivery of a packet. The source IP address of this packet will be the IP address of host A (11.8.8.11) and the destination IP address of this packet will be the IP address of host B (22.7.7.22).
...
So host A will first go through the ARP process to resolve R1’s MAC address. The ARP request will have an L2 header with a source MAC address of host A and a destination MAC address of all Fs – that’s the broadcast MAC address. This will tell the network that this ARP request should be sent to everybody in this network.
...
Then the switch will look at the destination MAC address. It is all Fs which tells the switch it should do the flooding action to send the frame out all ports except the port the frame was received on. That frame will be sent out port 5 where it will arrive on R1. Upon arriving to R1, R1 will get to learn the ARP mapping of the sender.
...
When the frame arrives at R1, R1 will strip the L2 header which has served its purpose. Then R1 will look at its routing table to determine where it’s going to send this packet next. The destination IP address of this packet is 22.7.7.22 which matches R1’s default route (0.0.0.0/0 Internet) – meaning, R1 knows that this packet will be sent across the Internet. Normally R1’s default route would point to a specific router on the Internet but for convenience we have abstracted this process out.

The packet will be passed from router to router across the Internet until it finally gets to R2. When it arrives at R2, R2 will look at its routing table to try and figure out how to deliver this packet. R2 will see that the destination IP address points to a directly connected route (22.7.7.0/24) and R2 will know it will deliver the packet to its final hop within the directly connected network that R2 is attached to.
...
The first step that host A will take to send data to host C – as you must have guessed by now – will be to construct an L3 header with the source IP address of host A and a destination IP address of host C.
...
Upon receiving the packet, R3 will look at the destination IP address and determine that it matches the directly connected route in R3’s routing table. This tells R3 that it’s going to have to put an L2 header onto this packet that matches the MAC address associated with the IP address 33.6.6.33.
...
Upon receiving the ARP request host C will learn the ARP mapping of the sender of that ARP request, i.e., the IP address 33.6.6.1 and the MAC address eee3. Then host C will put together the ARP response with an L2 header with a source MAC address of host C and a destination MAC address of R3.
...
- Data delivery relies on three core tables:
    - The MAC address table maps switch ports to MAC addresses and enables a switch to forward frames efficiently.
    - The ARP table maps IP addresses to MAC addresses, allowing a device to construct the correct Layer 2 header for the next hop.
    - The routing table maps destination networks to next‑hop IP addresses (or direct connections) so a router can forward packets towards their final destination.
- Network devices follow consistent packet‑processing rules. Regardless of whether a device is a host, switch, or router, its behavior is predictable: a switch learns and forwards, a host resolves gateways with ARP before sending, and a router strips Layer 2 headers, consults its routing table, and re‑encapsulates for the next hop.
- The communication process is layered. Layer 3 (IP) provides end to end host addressing; Layer 2 (Ethernet + ARP) handles hop by hop delivery; and the transport layer (Layer 4) ensures process‑to‑process delivery using ports, with optional reliability (TCP) or lightweight service (UDP).
- DNS and HTTP illustrate the client‑server model. Browsing to a website typically involves two phases: first, a DNS query to resolve the domain name into an IP address; second, an HTTP request to that IP address. The web server then returns the requested content over a TCP connection.
- Routing tables must be populated in advance, while ARP and MAC tables are built dynamically. Default routes and directly connected networks must already exist in the routing table; ARP and MAC address table entries are learned automatically as frames traverse the network.
- The same principles scale to networks of any size. Once you understand how these tables function in a simple topology, you can trace packet delivery through networks with hundreds of devices using the same logical steps.
