...
An effective wireless security strategy includes a method to authenticate clients and a method to provide data privacy and integrity. These two types of methods are listed in the leftmost column. The following table breaks down the core components of Wi-Fi security, explaining common methods for verifying identity (authentication) and for protecting data (privacy and integrity).

| Method Category                 | Mechanism      | Full Name & Description                                                                                                                                                   | Key Characteristics & Security Assessment                                                                                                                |
| ------------------------------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Authentication Methods**      | **Open**       | <p><strong>Open Authentication</strong><br>No real authentication. The network is publicly accessible.</p>                                                                | <p>• <strong>No Verification.</strong><br>• <strong>Use Case:</strong> Public hotspots. Often used with a captive portal (e.g., a login web page).</p>   |
|                                 | **WEP**        | <p><strong>Wired Equivalent Privacy</strong><br>Uses a static, pre-shared key for both authentication and encryption.</p>                                                 | <p>• <strong>Deeply Flawed &#x26; Deprecated.</strong><br>• <strong>Weakness:</strong> Static keys are easily cracked. <strong>Never use.</strong></p>   |
|                                 | **802.1X/EAP** | <p><strong>Extensible Authentication Protocol</strong><br>A framework for strong authentication, often using a RADIUS server.</p>                                         | <p>• <strong>Enterprise-Grade.</strong><br>• Provides centralized user authentication. Various EAP types exist, with different security properties.</p>  |
|                                 | ↳ **LEAP**     | <p><strong>Lightweight EAP</strong><br>An early Cisco protocol.</p>                                                                                                       | <p>• <strong>Deprecated &#x26; Vulnerable.</strong><br>• Uses dynamic WEP keys, but is susceptible to dictionary attacks.</p>                            |
|                                 | ↳ **EAP-FAST** | <p><strong>EAP Flexible Authentication by Secure Tunneling</strong><br>Uses a Protected Access Credential (PAC) for fast, secure authentication without certificates.</p> | <p>• <strong>Secure.</strong><br>• Good for environments where deploying client certificates is difficult.</p>                                           |
|                                 | ↳ **PEAP**     | <p><strong>Protected EAP</strong><br>Creates a TLS tunnel to protect the inner authentication method (e.g., MS-CHAPv2).</p>                                               | <p>• <strong>Very Common &#x26; Secure.</strong><br>• Only the server is authenticated with a digital certificate, simplifying client setup.</p>         |
|                                 | ↳ **EAP-TLS**  | <p><strong>EAP Transport Layer Security</strong><br>Uses TLS with digital certificates on both the client and server.</p>                                                 | <p>• <strong>Gold Standard for Security.</strong><br>• Provides mutual authentication. Most secure but requires a PKI to manage client certificates.</p> |
| **Privacy & Integrity Methods** | **TKIP**       | <p><strong>Temporal Key Integrity Protocol</strong><br>Was a temporary wrapper to fix WEP's flaws without new hardware.</p>                                               | <p>• <strong>Deprecated.</strong><br>• Used in WPA. Now considered weak and has been phased out in modern standards.</p>                                 |
|                                 | **CCMP**       | <p><strong>Counter Mode with Cipher Block Chaining Message Authentication Code Protocol</strong><br>Based on the robust AES cipher.</p>                                   | <p>• <strong>Strong &#x26; Widely Used.</strong><br>• The mandatory security protocol for WPA2. Provides confidentiality and integrity.</p>              |
|                                 | **GCMP**       | <p><strong>Galois/Counter Mode Protocol</strong><br>A more efficient and secure cipher suite.</p>                                                                         | <p>• <strong>Modern &#x26; Robust.</strong><br>• The preferred protocol for WPA3. Offers better performance and security than CCMP.</p>                  |
