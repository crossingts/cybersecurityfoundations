...
**Answer:** A basic hash only verifies that the message was not corrupted in transit; it cannot prevent malicious tampering. An active attacker (e.g., a man-in-the-middle) can modify the message, calculate a new hash of the altered message, and send both to the receiver. The receiver's verification will succeed because the new hash matches the modified message, falsely confirming integrity.
...
List the four key characteristics of an industry-grade hashing algorithm.
