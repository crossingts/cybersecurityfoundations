...
This type of encryption works by scrambling the text cipher according to some mathematical function – e.g., shifting the letters forward in this case.

In cryptographic encryption you need both an algorithm and a key.

* Algorithms provide the instructions and mathematical operations used to transform plain text into cipher text (the scrambled form). Different algorithms offer varying levels of security and are suited for different purposes.&#x20;
...
In the overly simplified example hello → lohel, “shifting the letters forward” represents the algorithm and “twice” or “two” represents the key used for that particular transformation.

A publicly known encryption algorithm (such as AES or RSA) is combined with a secret key to ensure strong encryption. Publicly known encryption algorithms are used because they have stood the test of time in terms of strength and usability. The secret key can be a randomly generated set of characters.
