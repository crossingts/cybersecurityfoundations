...
| **Vulnerability**                         | **CVSS**       | **Exploitability** | **Impact** | **Attack Target**                          | **Attack Vector**                                          | **Example Exploit**                                      | **Mitigation Strategies**                                                                                                 |
| ----------------------------------------- | -------------- | ------------------ | ---------- | ------------------------------------------ | ---------------------------------------------------------- | -------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| **Buffer Overflows**                      | 9.8 (Critical) | Moderate-High      | Critical   | Applications, OS, Services                 | Crafted input overflowing memory, executing shellcode      | EternalBlue (WannaCry)                                   | ▶ Use memory-safe languages (Rust, Go).<br>▶ Enable DEP/ASLR.<br>▶ Patch OS/libc regularly.                               |
| **Injection Flaws** (SQLi, XSS)           | 9.8 (Critical) | High               | Critical   | Web apps, APIs, Databases                  | SQLi, XSS, Command Injection                               | Equifax (SQLi)                                           | ▶ Parameterized queries.<br>▶ Input sanitization.<br>▶ WAF rules (e.g., ModSecurity).                                     |
| **Misconfigurations (Insecure Defaults)** | 9.0 (High)     | **Very High**      | High       | Servers, Cloud, IoT, APIs, Network Devices | Default credentials, open ports, exposed interfaces        | AWS S3 leaks, Jenkins RCE                                | ▶ Automated scanning (Chef, Ansible).<br>▶ Least-privilege access.<br>▶ Disable default credentials.                      |
| **Kernel Flaws**                          | 8.8 (High)     | Moderate           | Critical   | OS (Linux/Windows/macOS)                   | Privilege escalation via kernel bugs, exploits             | Dirty Pipe (CVE-2022-0847)                               | ▶ Immediate kernel patching.<br>▶ Restrict root access.<br>▶ Use SELinux/AppArmor.                                        |
| **Vulnerable Components**                 | 9.1 (Critical) | **Very High**      | Critical   | Libraries, Frameworks                      | Exploiting known CVEs in dependencies                      | Log4Shell, Struts (Equifax)                              | ▶ SBOM (Software Bill of Materials).<br>▶ Automated dependency updates (Dependabot).                                      |
| **Security Misconfigurations**            | 8.5 (High)     | High               | High       | Cloud, Servers, Containers                 | Exposed admin interfaces, verbose errors                   | Kubernetes API exposure                                  | ▶ CIS benchmarks.<br>▶ Regular audits with OpenSCAP.<br>▶ Disable debug modes.                                            |
| **Broken Authentication**                 | 8.8 (High)     | High               | High       | Web apps, APIs                             | Credential stuffing, session hijacking                     | Facebook token hijacking, OAuth misconfigurations        | ▶ MFA enforcement.<br>▶ Rate-limiting login attempts.<br>▶ OAuth 2.0 hardening.                                           |
| **SSRF**                                  | 8.7 (High)     | Moderate-High      | High       | Cloud, Internal Networks                   | Forging requests from the server                           | Capital One breach, AWS metadata theft                   | ▶ Network segmentation.<br>▶ Block internal IPs in requests.<br>▶ Use allowlists for URLs.                                |
| **Insufficient Input Validation**         | 8.1 (High)     | High               | High       | Web apps, APIs, Databases                  | SQLi, XSS, Command Injection                               | Heartbleed (OpenSSL)                                     | ▶ Input length/type checks.<br>▶ Fuzz testing (AFL).<br>▶ Zero-trust input models.                                        |
| **Race Conditions**                       | 7.5 (High)     | Hard               | High       | OS, Applications, Concurrent Systems       | TOCTOU (Time-of-Check to Time-of-Use) attacks              | Dirty COW (Linux)                                        | ▶ Atomic operations.<br>▶ File-locking mechanisms.<br>▶ TOCTOU checks.                                                    |
| **Unrestricted File Uploads**             | 8.0 (High)     | Moderate           | High       | Web apps                                   | Uploading malicious executables                            | WordPress malware uploads, Web shell uploads             | ▶ File type verification (magic numbers).<br>▶ Store uploads outside webroot.<br>▶ Scan with ClamAV.                      |
| **XSS**                                   | 7.5 (High)     | **Very High**      | Moderate   | Web apps, APIs                             | Cross-site scripting attacks                               | Tesla infotainment XSS                                   | ▶ CSP headers.<br>▶ Output encoding (OWASP ESAPI).<br>▶ DOM sanitization.                                                 |
| **Symbolic Links**                        | 7.1 (High)     | Moderate           | High       | File systems, Privileged apps              | Tricking apps into writing to sensitive files              | Docker breakout                                          | ▶ Disable symlink following.<br>▶ chroot/jail environments.<br>▶ Use <code>openat()</code> safely.                        |
| **Weak Credentials**                      | 7.5 (High)     | **Very High**      | High       | IoT, Web apps, Systems                     | Default/weak password exploitation                         | Mirai botnet (IoT)                                       | ▶ Password policies (12+ chars).<br>▶ Block common passwords.<br>▶ Certificate-based auth.                                |
| **Incorrect File Permissions**            | 7.8 (High)     | Moderate           | High       | OS, Databases, Apps, File Systems          | Unauthorized access/modification, reading restricted files | MongoDB ransomware                                       | ▶ <code>chmod 600</code> for sensitive files.<br>▶ Regular <code>auditd</code> checks.<br>▶ Principle of least privilege. |
| **Insecure Direct Object Refs.**          | 6.5 (Medium)   | High               | High       | Web apps, APIs                             | Manipulating object references                             | Accessing other users' data via ID parameter tampering   | ▶ Implement indirect reference maps, enforce authorization checks on every request.                                       |
| **File Descriptor Leaks**                 | 6.5 (Medium)   | Low                | High       | OS, Applications, Running Processes        | Exploiting unclosed file handles                           | Heartbleed                                               | ▶ Secure coding (close handles).<br>▶ Static analysis (Coverity).<br>▶ Memory-safe languages.                             |
| **Missing Encryption**                    | 6.8 (Medium)   | Low                | High       | Databases, Networks                        | Sniffing plaintext data                                    | FTP credentials intercepted, unencrypted medical records | ▶ TLS 1.3+ enforcement.<br>▶ Encrypt data at rest (AES-256).<br>▶ HSM for keys.                                           |
