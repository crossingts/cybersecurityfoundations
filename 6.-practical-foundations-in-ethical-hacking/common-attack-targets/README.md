...
| **SQL Injection**                         | ▶ **Burp Suite / OWASP ZAP** (intercepting proxy)  <br>▶ **SQLMap** (automated detection)  <br>▶ **Semgrep** (code analysis)            | ▶ **SQLMap** (automated exploitation)  <br>▶ **Burp Suite Repeater** (manual testing)  <br>▶ Custom scripts       | **1. CONTAIN:** Deploy virtual patch (WAF like ModSecurity). **2. ERADICATE:** Fix code: parameterized queries. **3. RECOVER:** Audit logs for stolen data; rotate exposed credentials.                               |
