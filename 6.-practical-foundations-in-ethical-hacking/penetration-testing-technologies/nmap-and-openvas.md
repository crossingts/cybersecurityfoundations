...
The CVE (Common Vulnerabilities and Exposures) Database is managed by MITRE Corporation. You can browse it at [https://cve.mitre.org/](https://cve.mitre.org/). The U.S. National Institute of Standards and Technology (NIST) provides the National Vulnerability Database (NVD), which enriches CVE entries with severity scores and patch information: [https://nvd.nist.gov/](https://nvd.nist.gov/). OpenVAS/GVM (Greenbone Vulnerability Management) uses these sources to update its own database.

**3. Penetration Testing (PT)**

Here is a comparison of the two tools in the context of a Penetration Testing (PT):
